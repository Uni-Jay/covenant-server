...
router.get('/books', async (req, res: Response) => {
  try {
    const books = [
      // Old Testament
  ...
  try {
    ...
    const [existing] = await pool.execute(
      `SELECT id FROM bible_notes 
       WHERE user_id = ? AND translation_id = ? AND book = ? AND chapter = ? AND verse = ?`,
      [userId, translationId, book, chapter, verse]
    ) as any;
    ...
    if (existing.length > 0) {
      ...
      await pool.execute(
        `UPDATE bible_notes 
         ...
         WHERE id = ?`,
        [note || null, highlightColor || null, existing[0].id]
      );
      ...
      const [result] = await pool.execute(
        `INSERT INTO bible_notes (user_id, translation_id, book, chapter, verse, note, highlight_color)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [userId, translationId, book, chapter, verse, note || null, highlightColor || null]
      ) as any;
...
router.delete('/notes/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Verify ownership
    const [notes] = await pool.execute(
      'SELECT id FROM bible_notes WHERE id = ? AND user_id = ?',
      [id, userId]
    ) as any;
    ...
    await pool.execute('DELETE FROM bible_notes WHERE id = ?', [id]);
...
router.get('/verse-of-the-day', async (req, res: Response) => {
  try {
