    ...
    io.on('connection', (socket) => {
        ...
        socket.on('call:invite', ({ callerId, callerName, callerPhoto, recipientIds, groupId, groupName, callType, roomId }) => {
            ...
            recipientIds.forEach((recipientId) => {
                const recipientSockets = Array.from(io.sockets.sockets.values()).filter((s) => s.userId === recipientId);
        ...
        socket.on('call:accept', ({ callerId, acceptorId, acceptorName, roomId }) => {
            ...
            const callerSockets = Array.from(io.sockets.sockets.values()).filter((s) => s.userId === callerId);
