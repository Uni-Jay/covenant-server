...
router.get('/', async (req, res) => {
  try {
    const [sermons]: any = await pool.execute('SELECT * FROM sermons ORDER BY date DESC');
...
router.post('/', authenticate, isAdmin, upload.fields([
  ...
  try {
    ...
    const [result]: any = await pool.execute(
      'INSERT INTO sermons (title, description, preacher, date, video_url, audio_url, pdf_url, thumbnail_url, category) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [title, description, preacher, date, videoUrl, audioUrl, pdfUrl, thumbnailUrl, category]
    );
