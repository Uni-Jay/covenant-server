...
import { BrevoMailOptions, sendBrevoTransactionalEmail } from './email.service';
...
const ORG_INFO_EMAIL = 'info@hocfam.org';
const EMAIL_FROM_ADDRESS = ORG_INFO_EMAIL;
const PASSWORD_RESET_FROM_ADDRESS = ORG_INFO_EMAIL;
const PASSWORD_RESET_TOKEN_TTL_MINUTES = Math.max(30, parseEnvNumber(process.env.PASSWORD_RESET_TOKEN_TTL_MINUTES, 180));
type MailPayload = BrevoMailOptions;
...
async function withTimeout<T>(promise: Promise<T>, label: string): Promise<T> {
  return Promise.race([
    ...
    new Promise<T>((_, reject) => {
      setTimeout(() => {
        const timeoutError = new Error(`${label} timed out after 15000ms`) as Error & {
          code?: string;
          command?: string;
        };
...
async function sendMailWithFallback(mailOptions: MailPayload, _label: string): Promise<boolean> {
  return sendBrevoTransactionalEmail(mailOptions);
}
...
export const sendWelcomeEmail = async (
  ...
  try {
    ...
    await sendMailWithFallback({
      from: `"Household Of Covenant And Faith Apostolic Ministry" <${EMAIL_FROM_ADDRESS}>`,
      to,
      subject,
      html: htmlContent,
    }, 'Welcome email');
...
<body>
  <div class="container">
    ...
    console.error('Password changed email error:', error instanceof Error ? error.message : String(error));
    return false;
  }
};
